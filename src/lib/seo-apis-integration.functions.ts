...
export const syncSearchConsoleData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const {
      ...
      service,
      credential,
    } = await resolveCentralSeoService("google-search-console");
      ...
      const response = await fetch(
        "https://www.googleapis.com/webmasters/v3/sites/searchanalytics/query",
        {
          ...
          body: JSON.stringify({
            ...
            rowLimit: 25000,
            ...(data.page_url && {
              filters: [{ dimension: "page", operator: "equals", expressions: [data.page_url] }],
            }),
          }),
        },
      );
...
export const runPageSpeedInsights = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const {
      ...
      service,
      credential: apiKey,
    } = await resolveCentralSeoService("google-pagespeed-insights");
    const startedAt = Date.now();
    ...
    try {
      ...
      await recordCentralSeoUsage(admin, service.id, "pagespeed_audit", response.status, startedAt);

      const speedResult: PageSpeedResult = {
        url: data.url,
        strategy,
        score_performance: metrics?.categories?.performance?.score
          ? Math.round(metrics.categories.performance.score * 100)
          : undefined,
        score_accessibility: metrics?.categories?.accessibility?.score
          ? Math.round(metrics.categories.accessibility.score * 100)
          : undefined,
        score_best_practices: metrics?.categories?.["best-practices"]?.score
          ? Math.round(metrics.categories["best-practices"].score * 100)
          : undefined,
        score_seo: metrics?.categories?.seo?.score
          ? Math.round(metrics.categories.seo.score * 100)
          : undefined,
    ...
    try {
      const response = await fetch("https://api.indexnow.org/indexnow", {
        ...
        body: JSON.stringify({
          ...
          key: credential,
          keyLocation:
            data.key_location || `https://${new URL(data.urls[0]).hostname}/indexnow-key.txt`,
          urlList: data.urls,
        }),
      });
      ...
      await recordCentralSeoUsage(admin, service.id, "url_submission", response.status, startedAt);
...
Context: Software Vala is a global SaaS marketplace with 12,000+ products across 80+ categories.`;
      ...
      await admin.from("seo_api_request_logs").insert({
        provider_id: (
          await admin.from("seo_api_providers").select("id").eq("name", "Google Gemini").single()
        ).data?.id,
...
export const validateSchema = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      await admin.from("seo_api_request_logs").insert({
        provider_id: (
          await admin.from("seo_api_providers").select("id").eq("name", "Schema.org").single()
        ).data?.id,
        endpoint: "/validate",
        method: "POST",
