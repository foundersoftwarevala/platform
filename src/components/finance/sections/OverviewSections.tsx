...
import { financeDayTotalsFn } from "@/lib/finance/finance.functions";
import {
  formatCurrency,
  formatCompact,
  formatDateTime,
...
const TITLES: Record<string, { title: string; description: string }> = {
  overview_total_balance: {
    title: "Total Balance",
    description: "Aggregate balance across all wallets in the system",
  },
  overview_today_inflow: {
    title: "Today's Income",
    description: "Credits received today across all channels",
  },
  overview_today_outflow: {
    title: "Today's Expense",
    description: "Debits and expenses recorded today",
  },
  overview_net_profit: { title: "Net Profit", description: "Revenue minus expenses over time" },
  overview_pending: {
    title: "Pending Payments",
    description: "Transactions and invoices awaiting settlement",
  },
};
...
export default function OverviewSections({ view }: { view: FinanceView }) {
  ...
  // Today is a business day decided on the server, in the configured timezone,
  // and summed over every transaction inside it. It used to be the viewer's own
  // calendar day applied to the newest three hundred rows the browser held.
  const dayTotalsFn = useServerFn(financeDayTotalsFn);
  const dayTotals = useQuery({
    queryKey: ["finance", "day-totals"],
    queryFn: () => dayTotalsFn({ data: {} }),
    staleTime: 60_000,
  });
  const todayInflow = dayTotals.data?.income ?? 0;
  const todayOutflow = dayTotals.data?.expense ?? 0;

  const pendingAmount = useMemo(() => {
    const pendingTxn = transactions
    ...
    const pendingPayouts = payouts
      .filter((p) => ["requested", "approved", "processing", "on_hold"].includes(p.status))
  ...
  const netProfitChange =
    latest && previous ? percentChange(Number(latest.profit), Number(previous.profit)) : 0;
  const inflowChange =
    latest && previous ? percentChange(Number(latest.inflow), Number(previous.inflow)) : 0;
  const outflowChange =
    latest && previous ? percentChange(Number(latest.outflow), Number(previous.outflow)) : 0;
  const revenueChange =
    latest && previous ? percentChange(Number(latest.revenue), Number(previous.revenue)) : 0;
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <XAxis
                  dataKey="date"
                  tickFormatter={(v) => formatDateTime(v).split(",")[0] ?? v}
                  tick={{ fontSize: 11 }}
                />
              ...
              <TableBody>
                {recentTransactions.map((t) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell
                      className={t.direction === "credit" ? "text-success" : "text-destructive"}
                    >
