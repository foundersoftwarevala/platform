...
const META: Record<
  string,
  {
    partnerType?: Commission["partner_type"];
    label: string;
...
> = {
  commission_franchise: {
    ...
    icon: Building2,
    description: "Commission ledger for franchise partners",
  },
  commission_reseller: {
    partnerType: "reseller",
    label: "Reseller Commission",
    icon: Users,
    description: "Commission ledger for reseller partners",
  },
  commission_influencer: {
    partnerType: "influencer",
    label: "Influencer Payout",
    icon: Megaphone,
    description: "Commission and payouts for influencer partners",
  },
  commission_rules: {
    label: "Commission Rules",
    icon: Settings,
    description: "Rate structure derived from live commission data",
  },
  commission_auto_deduct: {
    label: "Auto Deduction",
    icon: Zap,
    description: "Commissions marked for automatic deduction / paid status",
  },
};
...
export default function CommissionSections({ view }: { view: FinanceView }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return list.filter(
      (c) => c.partner_name.toLowerCase().includes(q) || c.partner_type.toLowerCase().includes(q),
    );
  }, [rows, search, view]);
  ...
  const stats = useMemo(() => {
    ...
    const avgRate = rows.length
      ? rows.reduce((sum, c) => sum + Number(c.rate_percent), 0) / rows.length
      : 0;
  ...
  const leaderboard = useMemo(() => {
    const map = new Map<
      string,
      { name: string; type: string; earned: number; paid: number; pending: number; count: number }
    >();
    for (const c of rows) {
      const entry = map.get(c.partner_name) ?? {
        name: c.partner_name,
        type: c.partner_type,
        earned: 0,
        paid: 0,
  ...
  const rateStructure = useMemo(() => {
    const map = new Map<
      string,
      { partnerType: string; rate: number; count: number; totalBase: number }
    >();
    for (const c of rows) {
    ...
    return Array.from(map.values()).sort(
      (a, b) => a.partnerType.localeCompare(b.partnerType) || a.rate - b.rate,
    );
  }, [rows]);
  ...
  const linkedPayouts = useMemo(
    () =>
      payoutRows.filter(
        (p) => p.recipient_type === "partner" || p.recipient_type === meta.partnerType,
      ),
    ...
    >
      <QueryState isLoading={commissionsState.isLoading} error={commissionsState.error}>
        <div className="space-y-6">
          ...
          {view === "commission_rules" && (
            <>
              <PanelCard title="Commission Rules">
                <QueryState
                  isLoading={rulesState.isLoading}
                  error={rulesState.error}
                  isEmpty={(rulesState.data ?? []).length === 0}
                  emptyLabel="No commission rule is configured"
                >
                    ...
                    {(rulesState.data ?? []).map((rule) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Priority {rule.priority} · {rule.currency}
                              {Number(rule.fixed_amount) > 0
                                ? ` · plus ${formatCompact(Number(rule.fixed_amount))} fixed`
                                : ""}
                        ...
                        <span className="text-lg font-bold text-success">
                          {formatPercent(Number(rule.rate_percent))}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <PanelCard title="Rates actually applied (from recorded commissions)">
                <QueryState
                  isLoading={false}
                  isEmpty={rateStructure.length === 0}
                  emptyLabel="No commission rates recorded yet"
                >
                  <div className="space-y-3">
                    {rateStructure.map((r) => (
                      <div
                        key={`${r.partnerType}-${r.rate}`}
                        className="flex items-center justify-between rounded-lg border border-border p-4"
                      >
                        <div className="flex items-center gap-4">
                          <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/15 text-primary">
                          ...
                          <div>
                            <p className="font-medium capitalize text-foreground">
                              {r.partnerType} partners
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {r.count} commission entries · Base volume{" "}
                              {formatCompact(r.totalBase)}
                            </p>
                          </div>
                        </div>
                        <span className="text-lg font-bold text-success">
          ...
          {view === "commission_auto_deduct" && (
            <PanelCard title="Linked Payouts">
              <QueryState
                ...
                isEmpty={linkedPayouts.length === 0}
                emptyLabel="No linked payouts"
              >
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
          ...
          >
            <QueryState
              isLoading={commissionsState.isLoading}
              error={commissionsState.error}
              isEmpty={isEmpty}
              emptyLabel="No commission records found"
            >
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
